...
posts.forEach((element) => {
    ...
    container.innerHTML += createTemplate(element);
});

// selezioniamo tutti i pulsanti con la classe
let likeButton = document.querySelectorAll(".like-button");
let likeNumber = document.querySelectorAll(".js-likes-counter");

for(let i = 0; i < likeButton.length; i++){
    const btn = likeButton[i];
    const counter = likeNumber[i];
    btn.addEventListener("click", function(e){
        e.preventDefault(); // impedisce alla pagina di tornare su 
        ...
        if(btn.classList.contains("like-button--liked")){
            ...
            container.innerText = likeNumber -1;
        }else{
